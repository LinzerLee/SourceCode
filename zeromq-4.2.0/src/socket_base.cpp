...
// #!# tid是指slot数组中的位置,sid是socket的唯一标识
...
int zmq::socket_base_t::connect (const char *addr_)
{
    ...
    if (protocol == "inproc") {
        ...
        // #!# new_pipes[0]对connect端来说属于inpipe
        // #!# new_pipes[1]对connect端来说属于outpipe(转移给bind端)
        rc = pipepair (parents, new_pipes, hwms, conflates);
        if (!conflate) {
        ...
        // #?# 怀疑以下代码有问题:
        // 当bind端不存在时,应该bool written = new_pipes [1]->write (&id);
        if (!peer.socket) {
...
// #!# 参数throttle_用于命令处理是否节流控制
// 当某个CPU平台获取时间戳是连接操作时,
// const uint64_t tsc = zmq::clock_t::rdtsc ();返回不为0,
// 此时若throttle_==true,则只有距离上次命令处
// 理超过3ms才会进行下次命令处理,从而减少
// 命令处理频率
