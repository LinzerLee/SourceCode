...
// connector端尝试连接bind端(A与B之间的连接不分先后)
// 1. 若bind端已经就绪(已经注册在端点池endpoints中),直接连接(connect_side)
// 2. 若bind端尚未就绪,先将连接信息挂在pend_connects池中,等待bind
//    就绪并主动连接本端(bind_side)
// 3. addr_为bind端地址,endpoint_为connect端地址
...
// bind端尝试连接addr_,前提是该addr_曾经发起过连接尝试,但那时候
// 本端尚未就绪
