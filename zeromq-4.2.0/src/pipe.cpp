...
//  #!# parents_[0]是connect端
//  #!# parents_[1]是bind端
//  #!# pipes_ [0]是属于connect端负责
//  #!# pipes_ [1]是属于bind端负责
...
zmq::pipe_t::pipe_t (object_t *parent_, upipe_t *inpipe_, upipe_t *outpipe_,
    ...
    in_active (true),
    out_active (true),
    // #!#inhwm_是本管道容量(实际上是本端option.recv和对端option.send之和)
    // #!#outhwm_是对端管道容量(实际上是本端option.send和对端option.recv之和)
    // #!#lwm用于定期通知对端本端接收的消息数量
    // #!#hwm用于write时,检查残留在管道中尚未被对端处理的消息数量
    hwm (outhwm_),
...
//  #!#本端不再负责inpipe,因为需要写线程将old inpipe的msg清除掉,
//  并减掉统计信息,所以hiccup操作会将inpipe的释放责任转移给对端
...
// #!# set_hwms_boost(peer.options.sndhwm, peer.options.rcvhwm);
// #!# in方向的boost是对端的sndhwm
// #!# out方向的boost是对端的rcvhwm
